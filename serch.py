...
def read_int_or_uint_N_big_endian_from_adarray8(data: npt.NDArray[np.uint8], dtype: type, position: int) -> Any:
    ...
    result = struct.unpack('>'+format_suffix, buffer.read(read_byte)) # type: ignore
...
def decode_node(data: npt.NDArray[np.uint8]) -> Node:
    ...
    keys: list[npt.NDArray[np.uint8]] = []
    number_of_keys = read_int_or_uint_N_big_endian_from_adarray8(data, np.int32, pos)
    pos += 32//8
    for _ in range(number_of_keys):
        key_size = read_int_or_uint_N_big_endian_from_adarray8(data, np.int32, pos)
        if (not key_size) or (key_size > 32):
            raise ValueError('Failed to parse key_size\nkey_size must be 32 or less')
        pos += 32//8
        keys.append(data[pos: pos+key_size])
        pos += key_size
    
    datas: list[NodeData] = []
    number_of_datas = read_int_or_uint_N_big_endian_from_adarray8(data, np.int32, pos)
    pos += 32//8
    for _ in range(number_of_datas):
        offset = read_int_or_uint_N_big_endian_from_adarray8(data, np.uint64, pos)
        pos += 64//8
        length = read_int_or_uint_N_big_endian_from_adarray8(data, np.int32, pos)
        pos += 32//8
        node_data = NodeData(offset, length)
        datas.append(node_data)
    
    subnode_addresses: list[np.uint64] = []
    number_of_subnode_addresses = 17#https://ltn.gold-usergeneratedcontent.net/searchlib.js の const B = 16から
    for _ in range(number_of_subnode_addresses):
        subnode_addresse = read_int_or_uint_N_big_endian_from_adarray8(data, np.uint64, pos)
        pos += 64//8
...
def get_node_at_address(field: str, address: np.uint64, galleries_index_version: str, serial: bool|None=None) -> Node:
    ...
    if (field == 'galleries'):
        url = 'https://'+'ltn.gold-usergeneratedcontent.net'+'/'+'galleriesindex'+'/galleries.'+galleries_index_version+'.index'
    else:
        raise ValueError('開発途中です')
    max_node_size = 464 #https://ltn.gold-usergeneratedcontent.net/searchlib.js の const max_node_size = 464から
    address_int = int(address)
    node_unit8 = get_url_at_range(url, (address_int, address_int+max_node_size-1))
